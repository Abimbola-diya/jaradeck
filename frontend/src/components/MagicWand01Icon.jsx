...
const MagicWand01Icon = forwardRef(
  ({ onMouseEnter, onMouseLeave, className, size = 28, loop = false, ...props }, ref) => {
    ...
    const { handleMouseEnter, handleMouseLeave } = useIconAnimation({
      controls,
      loops: !loop, // in loop mode the stop is handled by unmount, not mouse leave
      onMouseEnter,
      onMouseLeave,
      ref,
    });

    // When loop=true, start animating immediately and keep going
    useEffect(() => {
      if (loop) {
    ...
    // Pick the right wand variant based on loop mode
    const activeWandVariants = loop ? wandVariantsLoop : wandVariants;

    return (
      <div
        className={className}
        onMouseEnter={loop ? undefined : handleMouseEnter}
        onMouseLeave={loop ? undefined : handleMouseLeave}
        {...props}
      >
        <svg
